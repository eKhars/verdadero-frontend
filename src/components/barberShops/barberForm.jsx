...
import { searchBarbersShopsRequest } from "../../api/barber";
import { useBarber } from "../../context/BarberContext";

function BarberForm() {
  const { user } = useAuth();
  const { setSearchBarbers } = useBarber();
  const [barberShopsData, setBarberShopsData] = useState([]);
  const [grupoActual, setGrupoActual] = useState(1);

  useEffect(() => {
    const getBarbers = async () => {
      setBarberShopsData([searchBarbersShopsRequest])
    };
    getBarbers();
  }, []);
            ...
            {barbersGrupo.map((barberShop) => ( 
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">{barberShop.createdAt}</p>
